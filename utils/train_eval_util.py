...
from dataloaders import StanfordCars, Food101, Boat29, OxfordIIITPet, Cub2011, \
            Cub100, OxfordIIITPet_18, Food101_50, Boat29_14, StanfordCars98
...
def set_val_loader(args, preprocess=None):
    ...
    dataset_classes = {
        ...
        "boat29": lambda root, transform: Boat29(root, split="test", download=True, transform=transform),
        ...
        "boat14_ID": lambda root, transform: Boat29_14(root, split="test", id=True, download=True, transform=transform),
...
def set_ood_loader_ImageNet(args, out_dataset, preprocess, root):
    ...
    custom_datasets = {
        ...
        'boat14_OOD': lambda root, transform: Boat29_14(root, split="test", id=False, download=True,transform=transform)
    }
...
def select_random_image_paths_from_loader(loader, image_label):
    # print("image_label:")
    # print(image_label)
    ...
    try:
        ...
        if hasattr(sample, 'impath'):
            # 情况 1：自定义 Dataset，返回的是 Datum，含有 impath 属性
            all_paths = [dataset[i].impath for i in range(len(dataset))]
        elif isinstance(sample, (tuple, list)) and len(sample) == 3 and isinstance(sample[2], str):
            # (image, label, path)
            all_paths = [dataset[i][2] for i in range(len(dataset))]
        else:
            raise ValueError("Unsupported dataset type. Cannot extract image paths.")
    except Exception as e:
        raise RuntimeError(f"Failed to extract image paths: {e}")

    image_paths = random.sample(all_paths, min(image_label, len(all_paths)))
    return image_paths