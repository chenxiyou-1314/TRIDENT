class PromptGenerator:
    def __init__(self):
        self.dataset_info = {
            ...
            'boat29': {'class_type': None, 'num_classes': 29},
            ...
            'boat14_ID': {'class_type': 'boat', 'num_classes': 14}
        }

    def _fine_grained_image_prompt(self, class_type, num_classes, class_info, envision_nums=50):
        return f"""Q: I will give you a picture, please tell me what type of picture it is. Provide one type that best matches the image, which is not in my dataset. If the type cannot be identified, generate a new type label that is not in the dataset. Please output strictly according to the format. In this case, the species in my dataset are: ['aircraft_carrier', 'bitumen', 'bulk_carrier', 'catamaran_yacht', 'chemical_tanker', 'container_ship']
A: The types of dog that are not in the dataset are:
- Firefighting

Q: I will give you a picture, please tell me what type of picture it is. Provide one type that best matches the image, which is not in my dataset. If the type cannot be identified, generate a new type label that is not in the dataset. Please output strictly according to the format. In this case, the species in my dataset are:{class_info}
A: The types of {class_type} that are not in the dataset are:
"""
    # def _fine_grained_image_prompt(self, class_type, num_classes, class_info, envision_nums=50):
    #     return f"""Q: I will give you a picture, please tell me what type {class_type} of picture it is. Provide one {class_type} type that best matches the image.
    #         - Firefighting
    #         """

#
#     def _fine_grained_prompt(self, class_type, num_classes, class_info, envision_nums=50):
#         return f"""Q: I have a dataset containing 3 unique species of dogs. I need a list of 10 distinct dog species that are NOT present in my dataset, and ensure there are no repetitions in the list you provide. For context, the species in my dataset are: ['husky dog', 'alaskan Malamute', 'cossack sled dog', 'golden retriever', 'German Shepherd', 'Beagle', 'Bulldog', 'Poodle', 'Dachshund', 'Doberman Pinscher']
...
# A: The other {envision_nums} {class_type} species not in the dataset are(Do not give a serial number before each type, only give '-'
# ):
# - Labrador Retriever
# - Rottweiler
# ....
...
"""
    ...
    def get_prompt(self, ood_task, in_dataset, class_info=None, envision_nums=50):
        ...
        prompt_dispatcher = {
            ...
            'fine_grained_image': self._fine_grained_image_prompt,
        }
