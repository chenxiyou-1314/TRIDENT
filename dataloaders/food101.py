...
class Food101_50(Food101):
    def __init__(
        ...
        selected_classes_file = os.path.join('data', 'Food-50', 'selected_50_classes.pkl')
        if os.path.exists(selected_classes_file):
            with open(selected_classes_file, 'rb') as f:
                selected_classes = pickle.load(f)
        else:
            selected_classes = random.sample(self.classes, 50)
...
if __name__ == "__main__":
    ...
    train_set = Food101(root = "/data/ccy/ccy-factory/datasets/id_data", split = "train", download = True)
    test_set = Food101(root = "/data/ccy/ccy-factory/datasets/id_data", split = "test")
