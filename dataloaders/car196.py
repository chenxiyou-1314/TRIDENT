...
class StanfordCars(VisionDataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[Any, Any, str]:
        """Returns pil_image, class_id, and image path for given index"""
        ...
        return pil_image, target, str(image_path)
...
class StanfordCars98(StanfordCars):
    def __init__(
        ...
        subset_classes_file = os.path.join('data', 'Stanford-Cars-98', "selected_98_classes.pkl")
        if os.path.exists(subset_classes_file):
            with open(subset_classes_file, 'rb') as f:
                stored_classes = pickle.load(f)
        else:
            stored_classes = random.sample(self.classes, 98)
